...
# Test page for patient report
@app.get("/test-patient-report", response_class=HTMLResponse)
def test_patient_report(request: Request):
    return templates.TemplateResponse("test_patient_report.html", {"request": request})
...
class ApproveStudentModel(BaseModel):
    student_id: str
    doctor_id: str
    approved: bool

@app.post("/api/v1/doctor/approve_student")
def api_approve_student(data: ApproveStudentModel):
    result = approve_student_round(data.student_id, data.doctor_id, data.approved)
    return result
